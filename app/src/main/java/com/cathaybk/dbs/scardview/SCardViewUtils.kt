 ...
 * Control the CardView direction and color of shadow
 * From: https://github.com/meetsl/SCardView-master
 */
class SCardViewUtils(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : FrameLayout(context, attrs, defStyleAttr) {
    ...
    private val mCardViewDelegate = object : SCardViewDelegate {
        ...
        override fun getUseCompatPadding(): Boolean {
            return this@SCardViewUtils.getUseCompatPadding()
        }

        override fun getPreventCornerOverlap(): Boolean {
            return this@SCardViewUtils.getPreventCornerOverlap()
        }

        override fun setShadowPadding(left: Int, top: Int, right: Int, bottom: Int) {
            mShadowBounds.set(left, top, right, bottom)
            super@SCardViewUtils.setPadding(
        ...
        override fun setMinWidthHeightInternal(width: Int, height: Int) {
            if (width > mUserSetMinWidth) {
                super@SCardViewUtils.setMinimumWidth(width)
            }
            if (height > mUserSetMinHeight) {
                super@SCardViewUtils.setMinimumHeight(height)
            }
        }
    ...
    init {
        val a = context.obtainStyledAttributes(
            attrs, R.styleable.SCardView, defStyleAttr,
            R.style.CardView
        )
        ...
        mContentPadding.left = a.getDimensionPixelSize(
            R.styleable.SCardView_contentPaddingLeft,
            defaultPadding
        )
        mContentPadding.top = a.getDimensionPixelSize(
            R.styleable.SCardView_contentPaddingTop,
            defaultPadding
        )
        mContentPadding.right = a.getDimensionPixelSize(
            R.styleable.SCardView_contentPaddingRight,
            defaultPadding
        )
        mContentPadding.bottom = a.getDimensionPixelSize(
            R.styleable.SCardView_contentPaddingBottom,
            defaultPadding
        )
        ...
        IMPL = if (cardCornerVisibility == NONE) {
            SCardViewNormalImpl()
        } else {
            SCardViewBaseImpl()
        }
        ...
        backgroundColor?.let {
            IMPL.initialize(
                mCardViewDelegate, context, it, radius,
