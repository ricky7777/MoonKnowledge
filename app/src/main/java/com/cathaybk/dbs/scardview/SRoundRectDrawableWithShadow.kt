...
import com.cathaybk.dbs.scardview.*
    ...
    * */
    ...
    private fun setShadowSize(shadowSize: Float, maxShadowSize: Float) {
        ...
        if (updateShadowSize < 0f) {
            throw IllegalArgumentException(
                "Invalid shadow size " + updateShadowSize
                        + ". Must be >= 0"
            )
        }
        if (updateMaxShadowSize < 0f) {
            throw IllegalArgumentException(
                "Invalid max shadow size " + updateMaxShadowSize
                        + ". Must be >= 0"
            )
        }
    ...
    private fun buildShadowCorners(cornerRadius: Float) {
        ...
        mCornerShadowPaint.shader = RadialGradient(
            0f, 0f, cornerRadius + mShadowSize,
        ...
        mEdgeShadowPaint.shader = LinearGradient(
            0f, -cornerRadius + mShadowSize, 0f,
    ...
    private fun buildComponents(bounds: Rect) {
        ...
        mCardBounds.set(
            bounds.left + mRawMaxShadowSize, bounds.top + verticalOffset,
    ...
    fun getMinHeight(): Float {
        val content = 2 * Math.max(
            mRawMaxShadowSize, mCornerRadius + mInsetShadow.toFloat()
